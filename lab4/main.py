    ...
    if request.method == 'POST':
        ...
        try:
            with db.connect().cursor(named_tuple=True) as cursor:
                ...
                user_data = cursor.fetchone()
                if not user_data:
                    flash('старый пароль введен не правильно', 'danger')
                if new_password == confirm_new_password:
                    if check_password(new_password) != 1:
                        ...
                        cursor.execute(query, (new_password, login))
                        db.connect().commit()
                        flash('Вы успешно прошли поменяли пароль', 'success')
                        return redirect(url_for('index'))
                else:
                    flash('старый и новый пароли не совпадают', 'danger')
        except:
            flash('произошла ошибка, пароль не сменился', 'danger')
